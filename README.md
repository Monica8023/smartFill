...
2. 保持默认的“自动识别并进入目标表单”。
3. 点击“扫描页面字段”。Worker 会根据任务字段推断登录或注册意图，点击唯一高置信入口，
   重新观察页面后将发现的字段载入表单。
4. 如果目标入口无法由字段意图唯一确定，任务会暂停并展示当前页面的入口候选。
5. 检查字段名称和敏感标记，填写账号数据，再启动浏览器任务。

需要强制点击特定入口时，可选择“按别名点击指定入口”并配置 `登录, Login, Sign in` 等
别名。如果发现多个同分候选，正式执行时会暂停并让操作员从当前页面快照中选择；不接受
CSS、XPath 或脚本。表单入口跳到其他域名或子域名时，目标 Origin 也必须加入
